...
#ifndef _FADE_STATUS_H
#define _FADE_STATUS_H
 ...
 */
  ...
  #define FADE_STATUS_PWM_BITS 10
#else
  //Change this to match the number of PWM bit on other devices
  #define FADE_STATUS_PWM_BITS 8
#endif
 ...
 *  
 *  Is done automatically according to #FADE_STATUS_PWM_BITS. A byte (uint8_t) is used for 8-bit or less. An unsigned int (uint16_t) is used for anything above. So limited to 16-bit PWM.
 *  
 *  @see FADE_STATUS_PWM_BITS
 */
...
/**
 *  @brief Maximum number of FadeStatus objects
 *  
 *  **Default** = 6, the number of hardware PWM pins on an Uno/Pro Mini/Nano
 */
#ifndef FADE_STATUS_MAX_LED
#define FADE_STATUS_MAX_LED  6
#endif

/**
 *  @brief The maximum brightness step of the PWM
 *  
 *  Automatically set depending on #FADE_STATUS_PWM_BITS. 255 for 8-bit.
 *  
 *  @warning Can't simply be increased for resolution! It depends on the resolution of the hardware used. Set it using #FADE_STATUS_PWM_BITS.
 *  
 *  @see FADE_STATUS_PWM_BITS
 */
#ifndef FADE_STATUS_RESOLUTION
#define FADE_STATUS_RESOLUTION ((1 <<FADE_STATUS_PWM_BITS) -1)
#endif

#include "FadeStatusGamma.h"

/**
 *  @brief Main class of the FadeStatus-library
 *  
 *  The FadeStatus class includes all the functions to easily fade a LED including a static function to update all constructed FadeStatus objects.
 *  
 *  @see set(), update()
 */
class FadeStatus{
  public:
    /**
     *  @brief Simple constructor of a FadeStatus object with gamma correction.
     *  
     ...
     *  
     *  @see FadeStatus(byte, const flvar_t*, flvar_t), update(), set(), on(), off()
     */
    FadeStatus(byte pin);
    
    /**
     *  @brief Constructor of a FadeStatus object
     *  
     *  @details Makes a object to fade a specific pin. You can make a object for each PWM pin you want to fade. Updating all objects is done by simply calling update()
     *  
     *  When created the default brightness is **0**. You can start at a different brightness by calling begin().
     ...
     *  
     *  FadeStatus ledCustom = {Pin, myGammaTable, 19};
     *  ```
     *  
     *  @note Make all objects global (or static if you really must) because a destruct will lead to errors!
     *  
     ...
     *  
     *  @see setGammaTable(), FadeStatus(byte), FadeStatus(byte, bool), update(), set(), on(), off()
     */
    FadeStatus(byte pin, const flvar_t* gammaLookup, flvar_t biggestStep);
    
    /**
     *  @brief Simple constructor of a FadeStatus object
     *  
     *  @details Simalair to FadeStatus(byte) but with the possibility to disable the use of a gamma table by specifying false as hasGammaTable.
     *  
     *  @param [in] pin           The PWM pin to fade with this object
     ...
     *  @see FadeStatus(byte)
     */
    FadeStatus(byte pin, bool hasGammaTable);
    
    /**
     *  @brief Simple destructor of a FadeStatus object
     *  
     *  @details Destroy your FadeStatus object and removes it from the FadeStatus::update() cycle.
     */
    ~FadeStatus();
     ...
     *  
     *  @note To make all the fading work you need to call FadeStatus::update() **often** in the loop()!
     *  
     *  @see done(), setTime()
     *  
     ...
     *  
     *  @details Let this FadeStatus object use a specific gamma table. This table **must** be put in PROGMEM to work. #flvar_t can be used as variable type to get a variable type that matches the (set) bit resolution of the PWM.
     *  
     *  ```C++
     *  //put gamma table in PROGMEM
     *  const flvar_t myGammaTable[20] PROGMEM = {....};
     ...
     *  
     *  By default a 101 steps (0-100 aka percentage) table is used with a gamma of 2,3. To generate a table with a different gamma you can use the provided Python script ('FadeStatus\extras\GammaTable.py'). (You need to install Python for it to work!) Call it like: `python GammaTable.py Gamma Steps PWMbits [VariableName]`. VariableName is optional. For example `python gamma.py 2.5 50 10` will result in a table with 50 steps (0 - 49) with gamma = 2,5 for a 10-bit PWM. This will be stored in gamma.h and can be copy pasted into your code.
     *  
     ...
     *  
     *  It's short for `setGammaTable(nullptr, FADE_STATUS_RESOLUTION)`
     *  
     *  @see setGammaTable(), FADE_STATUS_RESOLUTION
     */
    void noGammaTable();
    ...
    /**
     *  @brief Updates all FadeStatus objects
     *  
     *  @details This is the core function of FadeStatus. Calling this function will check each object of FadeStatus to see if the brightness needs changing (fade). 
     *  
     *  It's a static function, you only need to call it once for all objects of FadeStatus. You can call it using the class name like:
     *  
     *  ```C++    
     *  loop(){
     *    FadeStatus::update();
     ...
     *  **default:** 50ms
     *  
     *  @warning Call this before setting a fading time (via setTime()). Changing the interval will change the fading time or **each** FadeStatus object.
     *  
     *  @see setTime()
     * 
  ...
  protected:
     ...
     *  
     *  @details Can't be called directly (it's protected). Instead call update() to update all FadeStatus objects.
     *  
     *  @see update()
     *  
     */
    ...
    static FadeStatus* _ledList[FADE_STATUS_MAX_LED]; //!< array of pointers to all FadeStatus objects
    static byte _ledCount; //!< Next number of FadeStatus object
    static unsigned int _interval; //!< Interval (in ms) between updates
    static unsigned int _millisLast; //!< Last time all FadeStatus objects where updated
};

inline flvar_t FadeStatus::getGamma(flvar_t step){
